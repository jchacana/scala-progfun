...
class BlurSuite extends FunSuite {
  ...
  test("boxBlurKernel should return the correct value on an interior pixel " +
    "of a 4x5 image with radius 2") {
    val src = new Img(4, 5)
    src(0, 0) = 0; src(1, 0) = 1; src(2, 0) = 2; src(3, 0) = 4
    src(0, 1) = 3; src(1, 1) = 4; src(2, 1) = 5; src(3, 1) = 3
    src(0, 2) = 6; src(1, 2) = 7; src(2, 2) = 8; src(3, 2) = 5
    src(0, 3) = 50; src(1, 3) = 11; src(2, 3) = 16; src(3, 3) = 7 
    src(0, 4) = 10; src(1, 4) = 12; src(2, 4) = 13; src(3, 4) = 15

    assert(boxBlurKernel(src, 1, 2, 2) === 10,
      s"(boxBlurKernel(1, 2, 2) should be 10, " +
        s"but it's ${boxBlurKernel(src, 1, 2, 2)})")
  }
